...
favouriteRouter.route('/:favoriteId')
    .options(cors.corsWithOptions, (req, res) => {
        res.sendStatus(200);
    })
    .get(cors.cors, authenticate.verifyUser, (req, res, next) => {
        ...
        Favorites.findOne({
            ...
            if (!favorite) {
                ...
                if (favorite.dishes.indexOf(req.params.favoriteId) < 0) {
                    ...
                    res.statusCode = 403;
                    res.setHeader("Content-Type", "text/plain");
                    res.end('Dish ' + req.params.favoriteId + 'already in favorites!');
                }
            }
        });
    })
    .put(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
        res.statusCode = 403;
        res.setHeader("Content-Type", "text/plain");
        res.end('PUT operation is not supported on /favourites/' + req.params.favoriteId);
    })
    .delete(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
        Favorites.findOne({
            user: req.user._id
            ...
            if (index >= 0) {
                ...
                res.statusCode = 404;
                res.setHeader("Content-Type", "text/plain");
                res.end('Dish ' + req.params.favoriteId + ' not in your favourites!');
            }
        });
    });
